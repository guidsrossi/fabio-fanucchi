...
function Tabela({ titulo, dados, onChange, disabled }: { titulo: string; dados: Linha[]; onChange: (linhas: Linha[]) => void; disabled: boolean }) {
  ...
  return <div className="overflow-x-auto rounded-2xl border border-slate-200 dark:border-white/10">
...
export default function FichaEstudantePanel({ estudantes, somenteLeitura }: { estudantes: Estudante[]; somenteLeitura: boolean }) {
  ...
  return <div className="mb-7 rounded-2xl border border-emerald-200 bg-emerald-50/40 p-4 dark:border-emerald-400/20 dark:bg-emerald-500/5 sm:p-5">
    <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><h3 className="font-bold text-slate-950 dark:text-white">Ficha do estudante</h3><p className="text-sm text-slate-500">Modelo digital baseado na frente e no verso da planilha enviada.</p></div><label className="grid min-w-64 gap-2 text-sm font-semibold">Estudante<select value={estudanteId} onChange={(e) => setEstudanteId(e.target.value)} className="rounded-xl border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-slate-900"><option value="">Selecione</option>{estudantes.map((e) => <option key={e.id} value={e.id}>{e.nome} - {e.turma}</option>)}</select></label></div>
    <div className="mb-5 flex gap-2" role="tablist"><button type="button" onClick={() => setAba('frente')} className={`rounded-xl px-4 py-2 font-semibold ${aba === 'frente' ? 'bg-emerald-700 text-white' : 'bg-white dark:bg-slate-900'}`}>Frente</button><button type="button" onClick={() => setAba('verso')} className={`rounded-xl px-4 py-2 font-semibold ${aba === 'verso' ? 'bg-emerald-700 text-white' : 'bg-white dark:bg-slate-900'}`}>Verso</button><span className="self-center text-sm text-slate-500">Os registros de tutoria continuam logo abaixo.</span></div>
    ...
    <div className="mt-5 flex items-center gap-3">{!somenteLeitura ? <button type="button" onClick={salvar} disabled={salvando || carregando || !estudanteId} className="rounded-xl bg-emerald-700 px-5 py-3 font-semibold text-white disabled:opacity-50">{salvando ? 'Salvando...' : 'Salvar ficha do estudante'}</button> : <p className="text-sm text-slate-500">Visualização da gestão: somente leitura.</p>}{mensagem ? <p className="text-sm font-medium text-slate-600 dark:text-slate-300">{mensagem}</p> : null}</div>
  </div>;
}
